...
data.rename(
    columns={
        ...
        'OutOfPocketExpenses',
        'DoesÂ\xa0this organization require an application or an interview/audition process to join?':
        'ApplicationProcess',
        'How frequently doesÂ\xa0this organization meet?':
        'MeetingFrequency',
        ...
        'HoursSpentByNonExec',
        'When doesÂ\xa0this organization elect new officers?':
        'WhenNewOfficersElected',
        'When doesÂ\xa0this organization do recruitment/membership intake?':
        'WhenRecruitment',
        ...
        'Are you the 2020-2021 incoming President OR a 2020-2021 returning member/officer?Â\xa0':
        'IncomingOrReturning',
...
#~~~~~APPLICATION PROCESS PIE CHARTS~~~~~~~~~
def my_autopct(pct):
    return ('%.2f' % pct) if pct > 2.5 else ''

iarr = list(map(int, data.index[data['ApplicationProcess'] == 'Interview/Audition'].tolist()))
iarr.remove(625)
iarr.remove(636)
ia = df.iloc[iarr]
labels1 = ['SAF Only', 'SAF and Fundraisers', 'Fundraisers Only', 'Law School Foundation and SAF', 'Dues and SAF', 'Alumni','None']
cur = ia['FundingSource'].value_counts()[0:7].tolist()
plt.pie(cur, labels=labels1, autopct=my_autopct)
plt.title('Interview/Audition')

iarr2 = list(map(int, data.index[data['ApplicationProcess'] == 'Both'].tolist()))
iarr2.remove(623)
iarr2.remove(626)
iarr2.remove(632)
iarr2.remove(642)
ia2 = df.iloc[iarr2]
labels2 = ['SAF Only', 'Fundraisers Only', 'None', 'Alumni and Fundraisers', 'Fundraisers and SAF']
cur2 = ia2['FundingSource'].value_counts()[0:5].tolist()
plt.pie(cur2, labels=labels2, autopct=my_autopct)
plt.title('Both Application and Interview/Audition')

iarr3 = list(map(int, data.index[data['ApplicationProcess'] == 'Application'].tolist()))
iarr3.remove(635)
iarr3.remove(637)
iarr3.remove(639)
ia3 = df.iloc[iarr3]
cur3 = ia3['FundingSource'].value_counts()[0:6].tolist()
labels3 = ['Fundraisers Only', 'SAF Only', 'Dues', 'Parents Fund and SAF', 'Fundraisers and SAF', 'National Organizations']
plt.pie(cur3, labels=labels3, autopct=my_autopct)
plt.title('Solely Application')

iarr4 = list(map(int, data.index[data['ApplicationProcess'] == 'Neither'].tolist()))
for x in range(622,644):
    if(x in iarr4):
        iarr4.remove(x)
ia4 = df.iloc[iarr4]
labels4 = ['SAF Only', 'Fundraisers Only', 'Dues', 'None', 'Law School Foundation','Fundraisers and SAF','Dues and Fundraisers','Grants','National Organizations','Alumni','Dues and SAF']
cur4 = ia4['FundingSource'].value_counts()[0:11].tolist()
cmap = plt.get_cmap('tab20')
colors2 = [cmap(i) for i in np.linspace(0, 1, 11)]
plt.pie(cur4, labels=labels4, autopct=my_autopct, colors=colors2)
plt.title('No Application')